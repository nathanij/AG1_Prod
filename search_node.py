...
class SearchNode:
    ...
    def get_state_array(self): # KEEP
        return self.state_.get_state_array()
    
    def get_active_player(self) -> bool: # KEEP
        return self.state_.get_active_player()
    
    def is_leaf(self) -> bool: # KEEP
    ...
    def add_visit(self): # KEEP
        self.visits_ += 1

    def branches(self) -> List[int]: # KEEP
        return self.children_.keys()
    
    def average_value(self) -> float: # KEEP
        return (self.total_value_ + self.value_) / (self.num_descendants_ + 1)

    def child_at(self, move: int) -> Type['SearchNode']:  # KEEP
        return self.children_[move]
    
    def add_child(self, move: int, strength: float, state: BoardState): # KEEP
        child_state = SearchNode(self, state, strength)
        self.children_[move] = child_state
        return child_state
    
    def set_value(self, value: float): # KEEP
        self.value_ = value

    # gathers and adds scores for generated leaves to their parent
    def reeval_leaf(self) -> Tuple[int, int]: # KEEP
        score = children = 0
    ...
    def reevaluate(self, added_score: float, added_descs: float): # KEEP
        self.total_value_ += added_score
        self.num_descendants_ += added_descs

    def ascend(self): # KEEP
